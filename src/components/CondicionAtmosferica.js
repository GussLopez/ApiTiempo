...
const CondicionAtmosferica = () => {
  ...
  return (
    <div className="contenedor">
      <h2>Estado del Tiempo en <span>México</span></h2>
            ...
            {loading ? (
              ...
              datos.map((ciudad, index) => (
                <div key={index} className="ciudad">
                  <p><span className='ciudad-nombre'>{ciudad.name}</span> - <i>{ciudad.skydescriptionlong || "Sin descripción"}</i></p>
                  ...
                  <p className='mt-4 mb-4'>Velocidad del viento: <span className='alert alert-danger'> {ciudad.windspeed || "No disponible"} km/h</span></p>
                </div>
              ))
            ) : (
